	...
	boost::optional<bool> bypass_document_validation)
{
	...
	try
	{
		...
		if (journal.is_initialized() || majority.is_initialized() || timeout.is_initialized()
			|| nodes.is_initialized() || tag.is_initialized() || acknowledge_level.is_initialized())
		{
											 ...
											 json request)
{
	...
	try
	{
		...
		find_options.collation(bsoncxx::from_json(request["options"].dump()));

		auto cursor = collection.find({bsoncxx::from_json(request["query"].dump())}, find_options);
