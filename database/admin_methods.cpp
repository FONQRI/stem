// header
#include "database/admin_methods.h"
												 ...
												 std::string &reply, std::string function_name)
{
	...
	try
	{
		...
		command_document.append(kvp("eval",
		...
		if (doc.view()["retval"].get_document().view()["isSuccessful"].get_bool())
		{
		...
		{
			reply = bsoncxx::to_json(doc.view()["retval"].get_document().view());
			return false;
		}
	}
										...
										std::string &reply, std::string function_name)
{
	...
	bsoncxx::types::b_document user_document = user_reply_document.view()["Response"].get_document();
	...
	try
	{
		...
		auto key = std::find_if(keys_array.begin(), keys_array.end(), [&request_key_name](auto &key) {
			return key.get_document().view()["name"].get_utf8().value.to_string()
				   == request_key_name;
		});
		...
		auto has_access = std::find_if(access_array.begin(), access_array.end(),
		...
		if (has_access == access_array.end())
		{
			...
			reply = core::reply::key_does_not_have_access_to_function(function_name);
			return false;
		}
		...
		else
		{
			...
			try
			{
				...
				find_document.append(
					kvp("keys.name", key_document["name"].get_utf8().value.to_string()));

				bsoncxx::builder::basic::document key_document{};

				bsoncxx::builder::basic::document update_document{};

				update_document.append(kvp("$inc", make_document(kvp("keys.$.request_per_day", 1))));
